...
static int input_strfile(strfile_t *str)
{
    ...
    rtklib_trace(4,"input_strfile: time=%s type=%d sat=%2d\n",time_str(str->time,3),
          type,str->sat);
    return type;
}
/* open stream file ----------------------------------------------------------*/
...
{
    rtklib_trace(3,"open_strfile: file=%s\n",file);
                           ...
                           const unsigned char *types, int sys, rnxopt_t *opt)
{
    ...
    rtklib_trace(3,"setopt_obstype: sys=%d\n",sys);
                        ...
                        gtime_t *time)
{
    ...
    rtklib_trace(3,"scan_obstype: file=%s, opt=%s\n",file,opt);
    ...
    if (abort) {
        rtklib_trace(2,"aborted in scan\n");
        return 0;
    }
    for (i=0;i<6;i++) for (j=0;j<n[i];j++) {
        rtklib_trace(2,"scan_obstype: sys=%d code=%s type=%d\n",i,code2obs(codes[i][j],NULL),types[i][j]);
    }
    for (i=0;i<6;i++) {
        ...
        for (j=0;j<n[i];j++) {
            rtklib_trace(3,"scan_obstype: sys=%d code=%s\n",i,code2obs(codes[i][j],NULL));
        }
    }
    return 1;
}
...
static void set_obstype(int format, rnxopt_t *opt)
{
    ...
    rtklib_trace(3,"set_obstype: format=%d\n",format);
...
/* open output files ---------------------------------------------------------*/
static int rnx_openfile(FILE **ofp, char *files[], const char *file,
                    const rnxopt_t *opt, nav_t *nav)
{
    char path[1024];
    int i;
    
    rtklib_trace(3,"rnx_openfile:\n");
...
/* close output files --------------------------------------------------------*/
static void rnx_closefile(FILE **ofp, const rnxopt_t *opt, nav_t *nav)
{
    int i;
    
    rtklib_trace(3,"rnx_closefile:\n");
...
static void convsbs(FILE **ofp, rnxopt_t *opt, strfile_t *str, int *n)
{
    ...
    if      (MINPRNSBS<=prn&&prn<=MAXPRNSBS) sys=SYS_SBS;
        ...
        rtklib_trace(2,"sbas message satellite error: prn=%d\n",prn);
        return;
    }
...
static void setapppos(strfile_t *str, rnxopt_t *opt)
{
    ...
    if (!pntpos(str->obs->data,str->obs->n,str->nav,&prcopt,&sol,NULL,NULL,
                msg)) {
        rtklib_trace(2,"point position error (%s)\n",msg);
        return;
    }
                     ...
                     char **ofile)
{
    ...
    rtklib_trace(3,"convrnx_s: sess=%d format=%d file=%s ofile=%s %s %s %s %s %s %s\n",
    ...
    /* open output files */
    if (!rnx_openfile(ofp,paths,path,opt,str->nav)) {
        for (i=0;i<MAXEXFILE;i++) free(epath[i]);
        free_strfile(str);
        return 0;
    }
...
extern int convrnx(int format, rnxopt_t *opt, const char *file, char **ofile)
{
    ...
    rtklib_trace(3,"convrnx: format=%d file=%s ofile=%s %s %s %s %s %s %s\n",
