...
static int decode_ss2llh(raw_t *raw)
{
    ...
    rtklib_trace(4,"decode_ss2llh: len=%d\n",raw->len);
    
    if (raw->len!=77) {
        rtklib_trace(2,"ss2 id#20 length error: len=%d\n",raw->len);
        return -1;
    }
...
static int decode_ss2ecef(raw_t *raw)
{
    ...
    rtklib_trace(4,"decode_ss2ecef: len=%d\n",raw->len);
    
    if (raw->len!=85) {
        rtklib_trace(2,"ss2 id#21 length error: len=%d\n",raw->len);
        return -1;
    }
...
static int decode_ss2meas(raw_t *raw)
{
    ...
    rtklib_trace(4,"decode_ss2meas: len=%d\n",raw->len);
    
    nobs=U1(p+2);
    if (17+nobs*11!=raw->len) {
        rtklib_trace(2,"ss2 id#23 message length error: len=%d\n",raw->len);
        return -1;
    }
    tow=floor(R8(p+3)*1000.0+0.5)/1000.0; /* rounded by 1ms */
    if (!adjweek(raw,tow)) {
        rtklib_trace(2,"ss2 id#23 message time adjustment error\n");
        return -1;
    }
    ...
    for (i=n=0,p+=11;i<nobs&&n<MAXOBS;i++,p+=11) {
        ...
        if (!(sat=satno(p[0]&0x20?SYS_SBS:SYS_GPS,prn))) {
            rtklib_trace(2,"ss2 id#23 satellite number error: prn=%d\n",prn);
            continue;
        }
...
static int decode_ss2eph(raw_t *raw)
{
    ...
    rtklib_trace(4,"decode_ss2eph: len=%d\n",raw->len);
    
    if (raw->len!=79) {
        rtklib_trace(2,"ss2 id#22 length error: len=%d\n",raw->len);
        return -1;
    }
    prn=(U4(p)&0x1F)+1;
    if (!(sat=satno(SYS_GPS,prn))) {
        rtklib_trace(2,"ss2 id#22 satellite number error: prn=%d\n",prn);
        return -1;
    }
    if (raw->time.time==0) {
        rtklib_trace(2,"ss2 id#22 week number unknown error\n");
        return -1;
    }
    ...
    if (decode_frame(buff   ,&eph,NULL,NULL,NULL,NULL)!=1||
        ...
        rtklib_trace(2,"ss2 id#22 subframe error: prn=%d\n",prn);
        return -1;
    }
...
static int decode_ss2sbas(raw_t *raw)
{
    ...
    rtklib_trace(4,"decode_ss2sbas: len=%d\n",raw->len);
    
    if (raw->len!=54) {
        rtklib_trace(2,"ss2 id#67 length error: len=%d\n",raw->len);
        return -1;
    }
    prn=U4(p+12);
...
static int decode_ss2(raw_t *raw)
{
    ...
    rtklib_trace(3,"decode_ss2: type=%2d\n",type);
    
    if (!chksum(raw->buff,raw->len)) {
        rtklib_trace(2,"ss2 message checksum error: type=%d len=%d\n",type,raw->len);
        return -1;
    }
    if (raw->outtype) {
...
{
    rtklib_trace(5,"input_ss2: data=%02x\n",data);
    ...
    if (raw->nbyte==4) {
        if ((raw->len=U1(raw->buff+3)+6)>MAXRAWLEN) {
            rtklib_trace(2,"ss2 length error: len=%d\n",raw->len);
            raw->nbyte=0;
            return -1;
        }
    }
