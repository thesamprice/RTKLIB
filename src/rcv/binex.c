...
{
    rtklib_trace(2,"binex 0x00-15: not supported message\n");
    return 0;
}
...
{
    rtklib_trace(2,"binex 0x00-7f: not supported message\n");
    return 0;
}
...
{
    rtklib_trace(2,"binex 0x01-00: not supported message\n");
    return 0;
}
...
static int decode_bnx_01_01(raw_t *raw, unsigned char *buff, int len)
{
    ...
    rtklib_trace(4,"binex 0x01-01: len=%d\n",len);
    ...
    if (len>=127) {
        ...
        rtklib_trace(2,"binex 0x01-01: length error len=%d\n",len);
        return -1;
    }
    if (!(eph.sat=satno(SYS_GPS,prn))) {
        rtklib_trace(2,"binex 0x01-01: satellite error prn=%d\n",prn);
        return -1;
    }
    eph.A=sqrtA*sqrtA;
...
static int decode_bnx_01_02(raw_t *raw, unsigned char *buff, int len)
{
    ...
    rtklib_trace(4,"binex 0x01-02: len=%d\n",len);
    ...
    if (len>=119) {
        ...
        rtklib_trace(2,"binex 0x01-02: length error len=%d\n",len);
        return -1;
    }
    if (!(geph.sat=satno(SYS_GLO,prn))) {
        rtklib_trace(2,"binex 0x01-02: satellite error prn=%d\n",prn);
        return -1;
    }
...
static int decode_bnx_01_03(raw_t *raw, unsigned char *buff, int len)
{
    ...
    rtklib_trace(4,"binex 0x01-03: len=%d\n",len);
    ...
    if (len>=98) {
        ...
        rtklib_trace(2,"binex 0x01-03 length error: len=%d\n",len);
        return -1;
    }
    if (!(seph.sat=satno(SYS_SBS,prn))) {
        rtklib_trace(2,"binex 0x01-03 satellite error: prn=%d\n",prn);
        return -1;
    }
...
static int decode_bnx_01_04(raw_t *raw, unsigned char *buff, int len)
{
    ...
    rtklib_trace(4,"binex 0x01-04: len=%d\n",len);
    ...
    if (len>=127) {
        ...
        rtklib_trace(2,"binex 0x01-04: length error len=%d\n",len);
        return -1;
    }
    if (!(eph.sat=satno(SYS_GAL,prn))) {
        rtklib_trace(2,"binex 0x01-04: satellite error prn=%d\n",prn);
        return -1;
    }
    eph.A=sqrtA*sqrtA;
    eph.iode=eph.iodc;
...
static int decode_bnx_01_05(raw_t *raw, unsigned char *buff, int len)
{
    ...
    rtklib_trace(4,"binex 0x01-05: len=%d\n",len);
    ...
    if (len>=117) {
        ...
        rtklib_trace(2,"binex 0x01-05: length error len=%d\n",len);
        return -1;
    }
    if (!(eph.sat=satno(SYS_CMP,prn))) {
        rtklib_trace(2,"binex 0x01-05: satellite error prn=%d\n",prn);
        return 0;
    }
    eph.A=sqrtA*sqrtA;
...
static int decode_bnx_01_06(raw_t *raw, unsigned char *buff, int len)
{
    ...
    rtklib_trace(4,"binex 0x01-06: len=%d\n",len);
    ...
    if (len>=127) {
        ...
        rtklib_trace(2,"binex 0x01-06: length error len=%d\n",len);
        return -1;
    }
    if (!(eph.sat=satno(SYS_QZS,prn))) {
        rtklib_trace(2,"binex 0x01-06: satellite error prn=%d\n",prn);
        return 0;
    }
    eph.A=sqrtA*sqrtA;
    eph.toe=gpst2time(eph.week,eph.toes);
                                           ...
                                           int sat, int nobs, obsd_t *data)
{
    ...
    rtklib_trace(5,"decode_bnx_7f_05_obs: sat=%2d nobs=%2d\n",sat,nobs);
    ...
    for (i=0;i<nobs;i++) {
        ...
        rtklib_trace(5,"(%d) CODE=%2d S=%d F=%02X %02X %02X %02X\n",i+1,
              code[i],slip[i],flags[0],flags[1],flags[2],flags[3]);
        rtklib_trace(5,"(%d) P=%13.3f L=%13.3f D=%7.1f SNR=%4.1f SCNT=%2d\n",
...
static int decode_bnx_7f_05(raw_t *raw, unsigned char *buff, int len)
{
    ...
    rtklib_trace(4,"decode_bnx_7f_05\n");
    ...
    for (i=0;i<nsat;i++) {
        ...
        rtklib_trace(5,"binex 0x7F-05 PRN=%3d SYS=%d NOBS=%d\n",prn,sys,nobs);
        ...
        if ((int)(p-buff)>len) {
            rtklib_trace(2,"binex 0x7F-05 length error: nsat=%2d len=%d\n",nsat,len);
            return -1;
        }
...
static int decode_bnx(raw_t *raw)
{
    ...
    rtklib_trace(5,"decode_bnx: rec=%02x len=%d\n",rec,len);
    ...
    if (cs1!=cs2) {
        rtklib_trace(2,"binex 0x%02X parity error CS=%X %X\n",rec,cs1,cs2);
        return -1;
    }
    if (raw->outtype) {
...
extern int input_bnx(raw_t *raw, unsigned char data)
{
    ...
    rtklib_trace(5,"input_bnx: data=%02x\n",data);
    ...
    if (raw->len-1>4096) {
        rtklib_trace(2,"binex length error: len=%d\n",raw->len-1);
        raw->nbyte=0;
        return -1;
    }
