...
static int decode_rxmrawx(raw_t *raw)
{
    ...
    if (raw->len<24) {
        rtk_trace(2,"ubx rxmrawx length error: len=%d\n",raw->len);
        return -1;
    }
    tow  =R8(p   ); /* rcvTow (s) */
    week =U2(p+ 8); /* week */
    ...
    if (raw->len<24+32*nmeas) {
        rtk_trace(2,"ubx rxmrawx length error: len=%d nmeas=%d\n",raw->len,nmeas);
        return -1;
    }
    if (week==0) {
        rtk_trace(3,"ubx rxmrawx week=0 error: len=%d nmeas=%d\n",raw->len,nmeas);
        return 0;
    }
    ...
    for (i=0,p+=16;i<nmeas&&n<MAXOBS;i++,p+=32) {
        ...
        if (!(sys=ubx_sys(gnss))) {
            rtk_trace(2,"ubx rxmrawx: system error gnss=%d\n", gnss);
            continue;
        }
        ...
        if (f==0||f>NFREQ+NEXOBS) {
            rtk_trace(2,"ubx rxmrawx signal error: sat=%2d sigid=%d\n",sat,sigid);
            continue;
        }
...
static int decode_nav(raw_t *raw, int sat, int off)
{
    ...
    if ((U4(p)>>24)==PREAMB_CNAV) {
        rtk_trace(3,"ubx rawsfrbx cnav not supported sat=%d prn=%d\n",sat,
              (U4(p)>>18)&0x3F);
        return 0;
    }
    ...
    id=(words[1]>>2)&7;
    if (id<1||5<id) {
        rtk_trace(2,"ubx rawsfrbx subfrm id error: sat=%2d id=%d len=%d\n",sat,id,
              raw->len);
        return -1;
    }
    for (i=0;i<10;i++) {
