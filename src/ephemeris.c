...
extern void alm2pos(gtime_t time, const alm_t *alm, double *rs, double *dts)
{
    ...
    if (n>=MAX_ITER_KEPLER) {
        rtk_trace(2,"alm2pos: kepler iteration overflow sat=%2d\n",alm->sat);
        return;
    }
                    ...
                    double *var)
{
    ...
    if (n>=MAX_ITER_KEPLER) {
        rtk_trace(2,"eph2pos: kepler iteration overflow sat=%2d\n",eph->sat);
        return;
    }
    sinE=sin(E); cosE=cos(E);
...
eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    if (iode>=0||j<0) {
        rtk_trace(3,"no broadcast ephemeris: %s sat=%2d iode=%3d\n",time_str(time,0),
              sat,iode);
        return NULL;
    }
    return nav->eph+j;
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        ...
        if (!ephclk(time[i],teph,obs[i].sat,nav,&dt)) {
            rtk_trace(3,"no broadcast clock %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
            continue;
        }
        ...
        if (!satpos(time[i],teph,obs[i].sat,ephopt,nav,rs+i*6,dts+i*2,var+i,
                    svh+i)) {
            rtk_trace(3,"no ephemeris %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
            continue;
        }
