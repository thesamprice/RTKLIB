...
static int decode_line2(const char *buff, tled_t *data)
{
    ...
    if (strcmp(satno,data->satno)) {
        rtklib_trace(2,"tle satno mismatch: %s %s\n",data->satno,satno);
        return 0;
    }
    if (data->n<=0.0||data->ecc<0.0) {
        rtklib_trace(2,"tle data error: %s\n",satno);
        return 0;
    }
    return 1;
}
...
static int add_data(tle_t *tle, const tled_t *data)
{
    ...
    if (tle->n>=tle->nmax) {
        ...
        if (!(tle_data=(tled_t *)realloc(tle->data,sizeof(tled_t)*tle->nmax))) {
            rtklib_trace(1,"tle malloc error\n");
...
extern int tle_read(const char *file, tle_t *tle)
{
    ...
    if (!(fp=fopen(file,"r"))) {
        rtklib_trace(2,"tle file open error: %s\n",file);
        return 0;
    }
...
extern int tle_name_read(const char *file, tle_t *tle)
{
    ...
    if (!(fp=fopen(file,"r"))) {
        rtklib_trace(2,"tle satellite name file open error: %s\n",file);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (i>=tle->n) {
            rtklib_trace(3,"no tle data: satno=%s desig=%s\n",satno,desig);
            continue;
        }
                   ...
                   double *rs)
{
    ...
    if (stat) {
        rtklib_trace(3,"no tle data: name=%s satno=%s desig=%s\n",name,satno,desig);
        return 0;
    }
    tutc=gpst2utc(time);
