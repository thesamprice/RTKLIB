    ...
    *var=0.0;
    ...
    if (iter>0) {
        if (testsnr(0,i,azel[1],obs->SNR[i]*0.25,&opt->snrmask)) {
            rtklib_trace(4,"snr mask: %s sat=%2d el=%.1f snr=%.1f\n",
...
{
    rtklib_trace(4,"ionocorr: time=%s opt=%d sat=%2d pos=%.3f %.3f azel=%.3f %.3f\n",
...
{
    rtklib_trace(4,"tropcorr: time=%s opt=%d pos=%.3f %.3f azel=%.3f %.3f\n",
                   ...
                   double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        if (i<n-1&&i<MAXOBS-1&&obs[i].sat==obs[i+1].sat) {
            rtklib_trace(2,"duplicated observation data %s sat=%2d\n",
        ...
        rtklib_trace(4,"sat=%2d azel=%5.1f %4.1f res=%7.3f sig=%5.3f\n",obs[i].sat,
                  ...
                  char *msg)
{
    ...
    rtklib_trace(3,"valsol  : n=%d nv=%d\n",n,nv);
                    ...
                    double *azel, int *vsat, double *resp, char *msg)
{
    ...
    rtklib_trace(3,"raim_fde: %s n=%2d\n",time_str(obs[0].time,0),n);
    ...
    for (i=0;i<n;i++) {
        ...
        if (!estpos(obs_e,n-1,rs_e,dts_e,vare_e,svh_e,nav,opt,&sol_e,azel_e,
                    vsat_e,resp_e,msg_e)) {
            rtklib_trace(3,"raim_fde: exsat=%2d (%s)\n",obs[i].sat,msg);
            continue;
        }
        ...
        if (nvsat<5) {
            rtklib_trace(3,"raim_fde: exsat=%2d lack of satellites nvsat=%2d\n",
                  obs[i].sat,nvsat);
            continue;
        }
        rms_e=sqrt(rms_e/nvsat);
        
        rtklib_trace(3,"raim_fde: exsat=%2d rms=%8.3f\n",obs[i].sat,rms_e);
    ...
    if (stat) {
        ...
        rtklib_trace(2,"%s: %s excluded by raim\n",tstr+11,name);
    }
    free(obs_e);
                  ...
                  char *msg)
{
    ...
    rtklib_trace(3,"pntpos  : tobs=%s n=%d\n",time_str(obs[0].time,3),n);
