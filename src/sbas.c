...
static int decode_sbstype1(const sbsmsg_t *msg, sbssat_t *sbssat)
{
    ...
    rtklib_trace(4,"decode_sbstype1:\n");
    ...
    rtklib_trace(5,"decode_sbstype1: nprn=%d iodp=%d\n",n,sbssat->iodp);
    return 1;
}
...
static int decode_sbstype2(const sbsmsg_t *msg, sbssat_t *sbssat)
{
    ...
    rtklib_trace(4,"decode_sbstype2:\n");
    ...
    rtklib_trace(5,"decode_sbstype2: type=%d iodf=%d\n",type,iodf);
    return 1;
}
...
static int decode_sbstype6(const sbsmsg_t *msg, sbssat_t *sbssat)
{
    ...
    rtklib_trace(4,"decode_sbstype6:\n");
    ...
    rtklib_trace(5,"decode_sbstype6: iodf=%d %d %d %d\n",iodf[0],iodf[1],iodf[2],iodf[3]);
    return 1;
}
...
static int decode_sbstype7(const sbsmsg_t *msg, sbssat_t *sbssat)
{
    ...
    rtklib_trace(4,"decode_sbstype7\n");
...
static int decode_sbstype9(const sbsmsg_t *msg, nav_t *nav)
{
    ...
    rtklib_trace(4,"decode_sbstype9:\n");
    
    if (!(sat=satno(SYS_SBS,msg->prn))) {
        rtklib_trace(2,"invalid prn in sbas type 9: prn=%3d\n",msg->prn);
        return 0;
    }
    ...
    rtklib_trace(5,"decode_sbstype9: prn=%d\n",msg->prn);
    return 1;
}
...
static int decode_sbstype18(const sbsmsg_t *msg, sbsion_t *sbsion)
{
    ...
    rtklib_trace(4,"decode_sbstype18:\n");
    ...
    rtklib_trace(5,"decode_sbstype18: band=%d nigp=%d\n",band,n);
    return 1;
}
...
static int decode_longcorr0(const sbsmsg_t *msg, int p, sbssat_t *sbssat)
{
    ...
    rtklib_trace(4,"decode_longcorr0:\n");
    ...
    rtklib_trace(5,"decode_longcorr0:sat=%2d\n",sbssat->sat[n-1].sat);
    return 1;
}
...
static int decode_longcorr1(const sbsmsg_t *msg, int p, sbssat_t *sbssat)
{
    ...
    rtklib_trace(4,"decode_longcorr1:\n");
...
static int decode_sbstype24(const sbsmsg_t *msg, sbssat_t *sbssat)
{
    ...
    rtklib_trace(4,"decode_sbstype24:\n");
...
{
    rtklib_trace(4,"decode_sbstype25:\n");
...
static int decode_sbstype26(const sbsmsg_t *msg, sbsion_t *sbsion)
{
    ...
    rtklib_trace(4,"decode_sbstype26:\n");
    ...
    rtklib_trace(5,"decode_sbstype26: band=%d block=%d\n",band,block);
    return 1;
}
...
extern int sbsupdatecorr(const sbsmsg_t *msg, nav_t *nav)
{
    ...
    rtklib_trace(3,"sbsupdatecorr: type=%d\n",type);
                     ...
                     sbs_t *sbs)
{
    ...
    if (!(fp=fopen(file,"r"))) {
        rtklib_trace(2,"sbas message file open error: %s\n",file);
        return;
    }
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (sbs->n>=sbs->nmax) {
            ...
            if (!(sbs_msgs=(sbsmsg_t *)realloc(sbs->msgs,sbs->nmax*sizeof(sbsmsg_t)))) {
                rtklib_trace(1,"readsbsmsg malloc error: nmax=%d\n",sbs->nmax);
                       ...
                       sbs_t *sbs)
{
    ...
    rtklib_trace(3,"sbsreadmsgt: file=%s sel=%d\n",file,sel);
...
extern int sbsreadmsg(const char *file, int sel, sbs_t *sbs)
{
    ...
    rtklib_trace(3,"sbsreadmsg: file=%s sel=%d\n",file,sel);
                      ...
                      const sbsigp_t **igp, double *x, double *y)
{
    ...
    rtklib_trace(4,"searchigp: pos=%.3f %.3f\n",pos[0]*R2D,pos[1]*R2D);
                      ...
                      const double *azel, double *delay, double *var)
{
    ...
    rtklib_trace(4,"sbsioncorr: pos=%.3f %.3f azel=%.3f %.3f\n",pos[0]*R2D,pos[1]*R2D,
    ...
    *delay=*var=0.0;
    ...
    if (err) {
        rtklib_trace(2,"no sbas iono correction: lat=%3.0f lon=%4.0f\n",posp[0]*R2D,
              posp[1]*R2D);
        return 0;
    }
    for (i=0;i<4;i++) {
    ...
    *delay*=fp; *var*=fp*fp;
    ...
    rtklib_trace(5,"sbsioncorr: dion=%7.2f sig=%7.2f\n",*delay,sqrt(*var));
    return 1;
}
                          ...
                          double *var)
{
    ...
    rtklib_trace(4,"sbstropcorr: pos=%.3f %.3f azel=%.3f %.3f\n",pos[0]*R2D,pos[1]*R2D,
          azel[0]*R2D,azel[1]*R2D);
                       ...
                       double *drs, double *ddts)
{
    ...
    rtklib_trace(3,"sbslongcorr: sat=%2d\n",sat);
    ...
    for (p=sbssat->sat;p<sbssat->sat+sbssat->nsat;p++) {
        ...
        if (fabs(t)>MAXSBSAGEL) {
            rtklib_trace(2,"sbas long-term correction expired: %s sat=%2d t=%5.0f\n",
        ...
        *ddts=p->lcorr.daf0+p->lcorr.daf1*t;
        ...
        rtklib_trace(5,"sbslongcorr: sat=%2d drs=%7.2f%7.2f%7.2f ddts=%7.2f\n",
    ...
    rtklib_trace(2,"no sbas long-term correction: %s sat=%2d\n",time_str(time,0),sat);
    return 0;
}
/* fast correction -----------------------------------------------------------*/
                       ...
                       double *prc, double *var)
{
    ...
    rtklib_trace(3,"sbsfastcorr: sat=%2d\n",sat);
    ...
    for (p=sbssat->sat;p<sbssat->sat+sbssat->nsat;p++) {
        ...
        *var=varfcorr(p->fcorr.udre)+degfcorr(p->fcorr.ai)*t*t/2.0;
        ...
        rtklib_trace(5,"sbsfastcorr: sat=%3d prc=%7.2f sig=%7.2f t=%5.0f\n",sat,
              *prc,sqrt(*var),t);
        return 1;
    }
    rtklib_trace(2,"no sbas fast correction: %s sat=%2d\n",time_str(time,0),sat);
    return 0;
}
                      ...
                      double *dts, double *var)
{
    ...
    rtklib_trace(3,"sbssatcorr : sat=%2d\n",sat);
    ...
    rtklib_trace(5,"sbssatcorr: sat=%2d drs=%6.3f %6.3f %6.3f dclk=%.3f %.3f var=%.3f\n",
                        ...
                        sbsmsg_t *sbsmsg)
{
    ...
    rtklib_trace(5,"sbsdecodemsg: prn=%d\n",prn);
