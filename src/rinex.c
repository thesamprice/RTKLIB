...
static void convcode(double ver, int sys, const char *str, char *type)
{
    ...
    rtklib_trace(3,"convcode: ver=%.2f sys=%2d type= %s -> %s\n",ver,sys,str,type);
}
/* decode obs header ---------------------------------------------------------*/
                        ...
                        char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta)
{
    ...
    rtklib_trace(4,"decode_obsh: ver=%.2f\n",ver);
    ...
    if      (strstr(label,"MARKER NAME"         )) {
        ...
        if (!(p=strchr(syscodes,buff[0]))) {
            rtklib_trace(2,"invalid system code: sys=%c\n",buff[0]);
            return;
        }
        i=(int)(p-syscodes);
        ...
        *tobs[i][nt]='\0';
        ...
        for (j=0;j<nt;j++) {
            ...
            rtklib_trace(2,"set default for unknown code: sys=%c code=%s\n",buff[0],
                  tobs[i][j]);
        }
    }
        ...
        if (strlen(buff)<=60) continue;
            ...
            *type=*(buff+20);
            ...
            switch (*(buff+40)) {
                ...
                default :
                    rtklib_trace(2,"not supported satellite system: %c\n",*(buff+40));
                    break;
            }
            continue;
        }
                           ...
                           int *flag, int *sats)
{
    ...
    rtklib_trace(4,"decode_obsepoch: ver=%.2f\n",ver);
    ...
    if (ver<=2.99) { /* ver.2 */
        ...
        *flag=(int)str2num(buff,28,1);
        ...
        if (str2time(buff,0,26,time)) {
            rtklib_trace(2,"rinex obs invalid epoch: epoch=%26.26s\n",buff);
            return 0;
        }
        ...
        *flag=(int)str2num(buff,31,1);
        ...
        if (buff[0]!='>'||str2time(buff,1,28,time)) {
            rtklib_trace(2,"rinex obs invalid epoch: epoch=%29.29s\n",buff);
            return 0;
        }
    }
    rtklib_trace(4,"decode_obsepoch: time=%s flag=%d\n",time_str(*time,3),*flag);
    return n;
}
/* decode obs data -----------------------------------------------------------*/
                          ...
                          obsd_t *obs)
{
    ...
    rtklib_trace(4,"decode_obsdata: ver=%.2f\n",ver);
    ...
    if (!obs->sat) {
        rtklib_trace(4,"decode_obsdata: unsupported sat sat=%s\n",satid);
        stat=0;
    }
    ...
    rtklib_trace(4,"decode_obsdata: time=%s sat=%2d\n",time_str(obs->time,0),obs->sat);
    return 1;
}
/* save slips ----------------------------------------------------------------*/
...
static int addobsdata(obs_t *obs, const obsd_t *data)
{
    ...
    if (obs->nmax<=obs->n) {
        ...
        if (!(obs_data=(obsd_t *)realloc(obs->data,sizeof(obsd_t)*obs->nmax))) {
            rtklib_trace(1,"addobsdata: memalloc error n=%dx%d\n",sizeof(obsd_t),obs->nmax);
                      ...
                      char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    ...
    for (p=opt;p&&(p=strchr(p,'-'));p++) {
        ...
        for (i=0;i<n;i++) {
            ...
            ind->shift[i]=shift;
            rtklib_trace(2,"phase shift: sys=%2d tobs=%s shift=%.3f\n",sys,
                  tobs[i],shift);
        }
    }
    ...
    for (i=0;i<n;i++) {
        ...
        rtklib_trace(4,"reject obs type: sys=%2d, obs=%s\n",sys,tobs[i]);
    }
    ind->n=n;
    
#if 0 /* for debug */
    for (i=0;i<n;i++) {
        rtklib_trace(2,"set_index: sys=%2d,tobs=%s code=%2d pri=%2d frq=%d pos=%d shift=%5.2f\n",
                      ...
                      char tobs[][MAXOBSTYPE][4], obs_t *obs)
{
    ...
    rtklib_trace(4,"readrnxobs: rcv=%d ver=%.2f tsys=%d\n",rcv,ver,tsys);
    ...
    rtklib_trace(4,"readrnxobs: nobs=%d stat=%d\n",obs->n,stat);
                      ...
                      eph_t *eph)
{
    ...
    rtklib_trace(4,"decode_eph: ver=%.2f sat=%2d\n",ver,sat);
    
    sys=satsys(sat,NULL);
    
    if (!(sys&(SYS_GPS|SYS_GAL|SYS_QZS|SYS_CMP))) {
        rtklib_trace(2,"ephemeris error: invalid satellite sat=%2d\n",sat);
        return 0;
    }
    *eph=eph0;
    ...
    if (eph->iode<0||1023<eph->iode) {
        rtklib_trace(2,"rinex nav invalid: sat=%2d iode=%d\n",sat,eph->iode);
    }
    if (eph->iodc<0||1023<eph->iodc) {
        rtklib_trace(2,"rinex nav invalid: sat=%2d iodc=%d\n",sat,eph->iodc);
    }
    return 1;
}
                       ...
                       geph_t *geph)
{
    ...
    rtklib_trace(4,"decode_geph: ver=%.2f sat=%2d\n",ver,sat);
    
    if (satsys(sat,NULL)!=SYS_GLO) {
        rtklib_trace(2,"glonass ephemeris error: invalid satellite sat=%2d\n",sat);
        return 0;
    }
    *geph=geph0;
    ...
    if (geph->frq<MINFREQ_GLO||MAXFREQ_GLO<geph->frq) {
        rtklib_trace(2,"rinex gnav invalid freq: sat=%2d fn=%d\n",sat,geph->frq);
    }
    return 1;
}
                       ...
                       seph_t *seph)
{
    ...
    rtklib_trace(4,"decode_seph: ver=%.2f sat=%2d\n",ver,sat);
    
    if (satsys(sat,NULL)!=SYS_SBS) {
        rtklib_trace(2,"geo ephemeris error: invalid satellite sat=%2d\n",sat);
        return 0;
    }
    *seph=seph0;
                       ...
                       int *type, eph_t *eph, geph_t *geph, seph_t *seph)
{
    ...
    rtklib_trace(4,"readrnxnavb: ver=%.2f sys=%d\n",ver,sys);
    ...
    while (fgets(buff,MAXRNXLEN,fp)) {
        ...
        if (i==0) {
            ...
            if (str2time(buff+sp,0,19,&toc)) {
                rtklib_trace(2,"rinex nav toc error: %23.23s\n",buff);
                return 0;
            }
            /* decode data fields */
...
static int add_eph(nav_t *nav, const eph_t *eph)
{
    ...
    if (nav->nmax<=nav->n) {
        ...
        if (!(nav_eph=(eph_t *)realloc(nav->eph,sizeof(eph_t)*nav->nmax))) {
            rtklib_trace(1,"decode_eph malloc error: n=%d\n",nav->nmax);
...
static int add_geph(nav_t *nav, const geph_t *geph)
{
    ...
    if (nav->ngmax<=nav->ng) {
        ...
        if (!(nav_geph=(geph_t *)realloc(nav->geph,sizeof(geph_t)*nav->ngmax))) {
            rtklib_trace(1,"decode_geph malloc error: n=%d\n",nav->ngmax);
...
static int add_seph(nav_t *nav, const seph_t *seph)
{
    ...
    if (nav->nsmax<=nav->ns) {
        ...
        if (!(nav_seph=(seph_t *)realloc(nav->seph,sizeof(seph_t)*nav->nsmax))) {
            rtklib_trace(1,"decode_seph malloc error: n=%d\n",nav->nsmax);
                      ...
                      nav_t *nav)
{
    ...
    rtklib_trace(3,"readrnxnav: ver=%.2f sys=%d\n",ver,sys);
...
static int readrnxclk(FILE *fp, const char *opt, int index, nav_t *nav)
{
    ...
    rtklib_trace(3,"readrnxclk: index=%d\n", index);
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (str2time(buff,8,26,&time)) {
            rtklib_trace(2,"rinex clk invalid epoch: %34.34s\n",buff);
            continue;
        }
        ...
        if (nav->nc>=nav->ncmax) {
            ...
            if (!(nav_pclk=(pclk_t *)realloc(nav->pclk,sizeof(pclk_t)*(nav->ncmax)))) {
                rtklib_trace(1,"readrnxclk malloc error: nmax=%d\n",nav->ncmax);
                     ...
                     obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    rtklib_trace(3,"readrnxfp: flag=%d index=%d\n",flag,index);
    ...
    rtklib_trace(2,"unsupported rinex type ver=%.2f type=%c\n",ver,*type);
    return 0;
}
                       ...
                       obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    rtklib_trace(3,"readrnxfile: file=%s flag=%d index=%d\n",file,flag,index);
    
    if (sta) init_sta(sta);
    
    /* uncompress file */
    if ((cstat=uncompress(file,tmpfile))<0) {
        rtklib_trace(2,"rinex file uncompact error: %s\n",file);
        return 0;
    }
    if (!(fp=fopen(cstat?tmpfile:file,"r"))) {
        rtklib_trace(2,"rinex file open error: %s\n",cstat?tmpfile:file);
        return 0;
    }
    /* read rinex file */
                    ...
                    sta_t *sta)
{
    ...
    rtklib_trace(3,"readrnxt: file=%s rcv=%d\n",file,rcv);
                   ...
                   nav_t *nav, sta_t *sta)
{
    ...
    rtklib_trace(3,"readrnx : file=%s rcv=%d\n",file,rcv);
...
static void combpclk(nav_t *nav)
{
    ...
    if (!(nav_pclk=(pclk_t *)realloc(nav->pclk,sizeof(pclk_t)*nav->nc))) {
        free(nav->pclk); nav->pclk=NULL; nav->nc=nav->ncmax=0;
        rtklib_trace(1,"combpclk malloc error nc=%d\n",nav->nc);
        return;
    }
    nav->pclk=nav_pclk;
    nav->ncmax=nav->nc;
    
    rtklib_trace(4,"combpclk: nc=%d\n",nav->nc);
}
...
extern int readrnxc(const char *file, nav_t *nav)
{
    ...
    rtklib_trace(3,"readrnxc: file=%s\n",file);
...
extern int open_rnxctr(rnxctr_t *rnx, FILE *fp)
{
    ...
    rtklib_trace(3,"open_rnxctr:\n");
    
    /* read rinex header from file */
    if (!readrnxh(fp,&ver,&type,&sys,&tsys,tobs,&rnx->nav,&rnx->sta)) {
        rtklib_trace(2,"open_rnxctr: rinex header read error\n");
        return 0;
    }
    if (!strchr(rnxtypes,type)) {
        rtklib_trace(2,"open_rnxctr: not supported rinex type type=%c\n",type);
        return 0;
    }
    rnx->ver=ver;
    rnx->type=type;
...
static void outobstype_ver2(FILE *fp, const rnxopt_t *opt)
{
    ...
    rtklib_trace(3,"outobstype_ver2:\n");
...
static void outobstype_ver3(FILE *fp, const rnxopt_t *opt)
{
    ...
    rtklib_trace(3,"outobstype_ver3:\n");
...
extern int outrnxnavb(FILE *fp, const rnxopt_t *opt, const eph_t *eph)
{
    ...
    rtklib_trace(3,"outrnxgnavb: sat=%2d\n",eph->sat);
...
extern int outrnxhnavb(FILE *fp, const rnxopt_t *opt, const seph_t *seph)
{
    ...
    rtklib_trace(3,"outrnxhnavb: sat=%2d\n",seph->sat);
