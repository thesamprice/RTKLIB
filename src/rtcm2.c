...
static int decode_type3(rtcm_t *rtcm)
{
    ...
    rtklib_trace(4,"decode_type3: len=%d\n",rtcm->len);
    ...
    if (i+96<=rtcm->len*8) {
        ...
        rtklib_trace(2,"rtcm2 3 length error: len=%d\n",rtcm->len);
        return -1;
    }
    return 5;
}
...
static int decode_type16(rtcm_t *rtcm)
{
    ...
    rtklib_trace(4,"decode_type16: len=%d\n",rtcm->len);
    ...
    rtklib_trace(3,"rtcm2 16 message: %s\n",rtcm->msg);
    return 9;
}
...
static int decode_type22(rtcm_t *rtcm)
{
    ...
    rtklib_trace(4,"decode_type22: len=%d\n",rtcm->len);
    ...
    if (i+24<=rtcm->len*8) {
        ...
        rtklib_trace(2,"rtcm2 22 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (i+24<=rtcm->len*8) {
...
extern int decode_rtcm2(rtcm_t *rtcm)
{
    ...
    rtklib_trace(3,"decode_rtcm2: type=%2d len=%3d\n",type,rtcm->len);
    
    if ((zcnt=getbitu(rtcm->buff,24,13)*0.6)>=3600.0) {
        rtklib_trace(2,"rtcm2 modified z-count error: zcnt=%.1f\n",zcnt);
        return -1;
    }
    adjhour(rtcm,zcnt);
    ...
    if (seqno-rtcm->seqno!=1&&seqno-rtcm->seqno!=-7) {
        rtklib_trace(2,"rtcm2 message outage: seqno=%d->%d\n",rtcm->seqno,seqno);
    }
    rtcm->seqno=seqno;
    rtcm->stah =stah;
    ...
    if (type==3||type==22||type==23||type==24) {
        if (rtcm->staid!=0&&staid!=rtcm->staid) {
           rtklib_trace(2,"rtcm2 station id changed: %d->%d\n",rtcm->staid,staid);
        }
        rtcm->staid=staid;
    }
    if (rtcm->staid!=0&&staid!=rtcm->staid) {
        rtklib_trace(2,"rtcm2 station id invalid: %d %d\n",staid,rtcm->staid);
        return -1;
    }
    switch (type) {
